...
**b) Retirement date mismatch**
If `scraped_dates[model_id]` exists AND `yt_dates[model_id]` exists, compare them.
If they differ, report it. The scraped date is the provider's official published date; the YT date is what was manually entered in YouTrack.
